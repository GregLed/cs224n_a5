...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        chars_idxs = self.words2charindices(sents)
        chars_idxs_padded = pad_sents_char(chars_idxs,self.char_pad)
        sents_var = torch.tensor(chars_idxs_padded, dtype=torch.long)
...
if __name__ == '__main__':
    ...
    # voc = VocabEntry()
    # sent = [['A', 'dog', 'is', 'nice'], ['What', 'to', 'doing']]
    # t = voc.to_input_tensor_char(sent, 'cpu')